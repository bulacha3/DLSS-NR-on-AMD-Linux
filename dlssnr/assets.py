...
def verify_assets(package_root):
    ...
    if not isinstance(files,dict):raise RuntimeError('Empty/invalid manifest')
    external=manifest.get('external_files',{})
    if not files and not isinstance(external,dict):raise RuntimeError('Invalid external asset manifest')
