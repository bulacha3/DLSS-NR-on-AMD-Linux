...
def _roots(explicit=None):
    candidates = [Path(explicit).expanduser()] if explicit is not None else [
        Path.home()/'.local/share/Steam', Path.home()/'.steam/steam', Path.home()/'.steam/root',
...
def _manifest_protons(path):
    """Resolve Steam tool registrations, including system VDF redirects."""
    try:
        entries = _get(_get(_kv(path), 'compatibilitytools', {}), 'compat_tools', {})
        if not isinstance(entries, dict):
            return []
        result = []
        for entry in entries.values():
            location = _get(entry, 'install_path')
            if not isinstance(location, str) or not location or '\0' in location:
                continue
            target = Path(location).expanduser()
            result.append(target if target.is_absolute() else path.parent / target)
        return result
    except (OSError, UnicodeError, RuntimeError, ValueError) as exc:
        warnings.warn(f'Steam tool registration skipped ({path}): {exc}', RuntimeWarning)
        return []


def discover_protons(steam_root=None):
    candidates, registrations = [], []
    compatibility_roots = list(SYSTEM_COMPATIBILITY_ROOTS)
    for root in _roots(steam_root):
        for library in _libraries(root):
            candidates.extend((library/'steamapps/common').glob('Proton*'))
            compatibility_roots.append(library/'compatibilitytools.d')
    for directory in dict.fromkeys(compatibility_roots):
        for entry in sorted(directory.glob('*')):
            if entry.is_dir():
                candidates.append(entry)
                registrations.extend(entry.glob('*.vdf'))
            elif entry.suffix.lower() == '.vdf' and entry.is_file():
                registrations.append(entry)
    for path in sorted(set(registrations)):
        candidates.extend(_manifest_protons(path))
    result = []
...
def validate_proton(path):
    ...
    advice = 'Choose a compatible Proton/Wine runner in your launcher and specify its path; no automatic downloads.'
    try:
